(function($) {
    // $(document).on('mouseover', '.vote-block a', function() {
    //     var $el = $(this);
    ...
    // }).on('mouseleave', '.vote-block a', function() {
    //     $('#rating-info').hide();
    // });
    $(document).on('click', '.vote-block span', function() {
        var $el = $(this);
        ...
        .done(function(data){
            ...
            if (pr === 'limit') {
                ...
                let star_count = Math.round(pr/20);
                let j = 1;
                $el.parent().find('span').each(function(i){

                    if (j <= star_count) $(this).addClass('orange rated');
                    if (j == star_count) $(this).addClass('active');

                    j++;
                });

                $($el).closest('.ratings').find('.legend').text(abs + ' ' + '('+npr+'%) '+ text);
            }
        })
